...
namespace SabberStoneBasicAI.AIAgents.manuelliebchen
{
	...
	class MyAgentLiebchen : AbstractAgent
	{
		...
		private Dictionary<PlayerTask, int> SimulateAll( POGame initgame, List<PlayerTask> opitons, int depth)
		{
			...
			foreach (KeyValuePair<PlayerTask, POGame> sim in sims) {
				...
				{
					score = SimulateAll( sim.Value, sim.Value.CurrentPlayer.Options(), depth - 1).ToList().Max(x => x.Value);
				}
		...
		{
			return new MyScore { Controller = ( playerid == state.CurrentPlayer.PlayerId ? state.CurrentPlayer : state.CurrentOpponent) }.Rate();
		}
